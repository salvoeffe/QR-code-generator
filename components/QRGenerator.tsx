...
export default function QRGenerator({ initialContentType = 'url', onContentTypeChange }: QRGeneratorProps) {
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      doc.addImage(base64, 'PNG', qrX, qrY, qrMm, qrMm);
      doc.setFontSize(9);
      doc.setTextColor(128, 128, 128);
      doc.text('Generated by generatemyqrcode.com', pageW / 2, pageH - 10, { align: 'center' });
      doc.save('qrcode.pdf');
      setJustDownloaded(true);
      if (downloadSuccessTimerRef.current) clearTimeout(downloadSuccessTimerRef.current);
      downloadSuccessTimerRef.current = setTimeout(() => setJustDownloaded(false), 2500);
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                <button
                  ...
                  disabled={justDownloaded || downloadingPdf}
                  className={`px-5 py-2.5 min-h-[44px] rounded-xl border font-medium transition-all duration-200 active:scale-[0.98] ${
                    justDownloaded || downloadingPdf
                      ? 'border-zinc-200 dark:border-zinc-600 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 cursor-default'
                      : 'border-zinc-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'
                  }`}
                >
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-500 text-center">
                PNG for screens · SVG for print · PDF for documents
              </p>
