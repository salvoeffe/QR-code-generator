...
const CARD_IMAGE_ASPECT = 'aspect-[2/1]';
...
export default function CategoryIcon({ category, className }: { category: Category; className?: string }) {
  const iconClass = className ?? 'w-10 h-10 text-zinc-400 dark:text-zinc-500';
  switch (category) {
    case 'Industry Guides':
      return <IconUtensils className={iconClass} />;
    case 'Troubleshooting':
      return <IconShield className={iconClass} />;
    case 'Marketing Tips':
      return <IconChart className={iconClass} />;
    case 'Guides & Comparisons':
